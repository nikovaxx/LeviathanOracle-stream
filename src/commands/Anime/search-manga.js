...
const { searchMangaCatalog, getMangaDetailsByMalId } = require('../../utils/API-services');
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      if (/^\d+$/.test(query)) {
        const m = await getMangaDetailsByMalId(query);
        ...
        return interaction.editReply(ui.interactionPrivate({
          ...
          image: m.images?.jpg?.large_image_url || m.images?.jpg?.image_url || m.images?.webp?.large_image_url || m.images?.webp?.image_url,
          color: '#00AE86'
        }));
      }

      const list = await searchMangaCatalog(query, 10);
      ...
      interaction.editReply(ui.interactionPrivate({ title: `Search: ${query.slice(0, 50)}`, fields, color: '#00AE86' }));
    } catch (e) {
      console.error(e);
      const err = 'Error fetching manga.';
  ...
  async autocomplete(interaction) {
    ...
    try {
      const list = await searchMangaCatalog(val, 20) || [];
